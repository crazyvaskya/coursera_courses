...
def run(host, port):
    ...
    try:
        ...
        if metrics != expected_metrics:
            print(
                f"client.get('*') вернул неверный результат. Ожидается: \
                {expected_metrics}. Получено: {metrics}")
            sys.exit(1)
    except Exception as err:
    ...
    try:
        ...
        if metrics != expected_metrics:
            print(
                f"client.get('k2') вернул неверный результат. Ожидается: \
                {expected_metrics}. Получено: {metrics}")
            sys.exit(1)
    except Exception as err:
    ...
    try:
        ...
        if result != {}:
            print(
                ...
                добавлен. Ожидается: пустой словарь. Получено: {result}")
            sys.exit(1)
    except Exception as err:
        print(
